...
#equilibrate
BAOAB!(p, q, m, params, 1.0, L, LJ_pot_der, dλsoft_pot_dr, γ, T, δt, eq_steps)

#Compute correlation time for U
Us = zeros(eval_steps)
@inbounds for i in ProgressBar(1:eval_steps)
    onestep_BAOAB!(p, q, m, params, 1.0, L, LJ_pot_der, dλsoft_pot_dr, γ, T, δt)
    Us[i] = Upi(q, params, 1.0, L, LJ_pot, λsoft_pot)
end
τ = autocorr_time(Us, 2000)
println("The autocorrelation time of the potential energy in this steup is approximately $(τ)")
sample_every = 2*ceil(Int, τ)
...
@inbounds for i in ProgressBar(1:n_samples)
    BAOAB!(p, q, m, params, 1.0, L, LJ_pot_der, dλsoft_pot_dr, γ, T, δt, sample_every)
...
savefig("Perf_Checks_plots/Temperature_Conservation_Check_$(ρ)_$(T).png")
...
savefig("Perf_Checks_plots/U_check_$(ρ)_$(T).png")
